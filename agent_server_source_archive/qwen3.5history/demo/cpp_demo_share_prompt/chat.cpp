...
bool Qwen3_5::check_stop(const std::string &text) {
...
void Qwen3_5::net_launch_decode(int idx, int kv_offset,
  ...
  // Short decode stages must read the persistent stage-0 KV cache rather
  // than their own otherwise-empty stage input memory.
...
void Qwen3_5::save_share_prompt() {
  share_length = history_length;
  if (past_key_bk.empty()) {
    past_key_bk.resize(NUM_LAYERS);
    past_value_bk.resize(NUM_LAYERS);
    for (int i = 0; i < NUM_LAYERS; i++) {
      // full attention layers only need the used part of the kv cache;
      // linear attention layers always keep the whole conv/recurrent state
      size_t key_bytes = is_FA(i) ? (size_t)share_length * KV_BYTES
                                  : bm_mem_get_device_size(past_key[i]);
      size_t value_bytes = is_FA(i) ? (size_t)share_length * KV_BYTES
                                    : bm_mem_get_device_size(past_value[i]);
      auto status =
          bm_malloc_device_byte(bm_handle, &past_key_bk[i], key_bytes);
      assert(BM_SUCCESS == status);
      status = bm_malloc_device_byte(bm_handle, &past_value_bk[i], value_bytes);
      assert(BM_SUCCESS == status);
    }
  }
  for (int i = 0; i < NUM_LAYERS; i++) {
    // d2d copies the whole src by default, so pass the backup size
    // explicitly (the backup of an fa layer is smaller than the full cache)
    d2d(past_key_bk[i], past_key[i], 0, bm_mem_get_device_size(past_key_bk[i]));
    d2d(past_value_bk[i], past_value[i], 0,
        bm_mem_get_device_size(past_value_bk[i]));
  }
  has_share_prompt = true;
}

void Qwen3_5::restore_share_prompt() {
  if (!has_share_prompt) {
    return;
  }
  for (int i = 0; i < NUM_LAYERS; i++) {
    d2d(past_key[i], past_key_bk[i]);
    d2d(past_value[i], past_value_bk[i]);
  }
  history_length = share_length;
}
...
void Qwen3_5::init(int dev_id, std::string model_path, std::string config_path,
                   bool do_sample_, int repetition_window_) {
...
void Qwen3_5::deinit() {
  for (auto &mem : past_key_bk) {
    bm_free_device(bm_handle, mem);
  }
  for (auto &mem : past_value_bk) {
    bm_free_device(bm_handle, mem);
  }
...
void Qwen3_5::forward_vit(const float *pixel_values,
...
int Qwen3_5::penalty_sample(bm_device_mem_t &logits_mem) {
  std::vector<bm_tensor_t> in_tensors;
  std::vector<bm_tensor_t> out_tensors;
