...
public:
  ...
  void clear_history();
  uint64_t save_snapshot();
  int restore_snapshot(uint64_t snapshot_id);
  void release_snapshot(uint64_t snapshot_id);
  void clear_snapshots();
  uint64_t estimate_snapshot_bytes() const;
  uint64_t snapshot_bytes(uint64_t snapshot_id) const;
  uint64_t total_snapshot_bytes() const { return snapshot_bytes_total; }
...
private:
  struct CacheSnapshot {
    std::vector<bm_device_mem_t> keys;
    std::vector<bm_device_mem_t> values;
...
private:
  ...
  std::unordered_map<uint64_t, CacheSnapshot> snapshots;
  uint64_t next_snapshot_id = 1;
  uint64_t snapshot_bytes_total = 0;
  bool initialized = false;
};

void Qwen3_5::init_tensors(const bm_net_info_t *net,
                           std::vector<bm_tensor_t> &in_tensors,
...
void Qwen3_5::net_launch(const bm_net_info_t *net,
                         const std::vector<bm_tensor_t> &in_tensors,
                         std::vector<bm_tensor_t> &out_tensors) {
...
void Qwen3_5::net_launch_decode(int idx, int kv_offset,
  ...
  // The persistent KV cache is owned by stage 0 (the maximum sequence
  // length).  Shorter decode stages have separate input_mems, so leaving the
  // default stage memory here makes them attend to an empty cache.  Rebind
  // both history inputs to correctly-sized views of the persistent cache.
  auto history_bytes = bm_mem_get_device_size(in_tensors[3].device_mem);
  in_tensors[3].device_mem = bm_mem_from_device(
      past_key[idx].u.device.device_addr, history_bytes);
  in_tensors[4].device_mem = bm_mem_from_device(
      past_value[idx].u.device.device_addr, history_bytes);
...
void Qwen3_5::d2d(bm_device_mem_t &dst, bm_device_mem_t &src, int offset,
                  int size) {
  if (!size) {
...
uint64_t Qwen3_5::estimate_snapshot_bytes() const {
  if (!support_history || history_length <= 0) {
    return 0;
  }
  const uint64_t cached_tokens = static_cast<uint64_t>(history_length - 1);
  uint64_t bytes = 0;
  for (int i = 0; i < NUM_LAYERS; i++) {
    if (is_FA(i)) {
      bytes += 2 * cached_tokens * static_cast<uint64_t>(KV_BYTES);
    } else {
      bytes += bm_mem_get_device_size(past_key[i]);
      bytes += bm_mem_get_device_size(past_value[i]);
    }
  }
  return bytes;
}

uint64_t Qwen3_5::save_snapshot() {
  if (!support_history || history_length <= 0 || token_length <= 0) {
    throw std::runtime_error("cannot snapshot an empty/non-history cache");
  }

  CacheSnapshot snapshot;
  snapshot.keys.resize(NUM_LAYERS);
  snapshot.values.resize(NUM_LAYERS);
  snapshot.history_length = history_length;
  snapshot.pending_token = visited_tokens[token_length - 1];
  snapshot.bytes = estimate_snapshot_bytes();
  int allocated_layers = 0;

  try {
    const int cached_tokens = history_length - 1;
    for (int i = 0; i < NUM_LAYERS; i++) {
      const uint64_t key_bytes = is_FA(i)
          ? static_cast<uint64_t>(cached_tokens) * KV_BYTES
          : bm_mem_get_device_size(past_key[i]);
      const uint64_t value_bytes = is_FA(i)
          ? static_cast<uint64_t>(cached_tokens) * KV_BYTES
          : bm_mem_get_device_size(past_value[i]);
      bm_status_t status = bm_malloc_device_byte(
          bm_handle, &snapshot.keys[i], key_bytes);
      if (status != BM_SUCCESS) {
        throw std::runtime_error("device allocation failed for snapshot key");
      }
      status = bm_malloc_device_byte(bm_handle, &snapshot.values[i], value_bytes);
      if (status != BM_SUCCESS) {
        bm_free_device(bm_handle, snapshot.keys[i]);
        throw std::runtime_error("device allocation failed for snapshot value");
      }
      allocated_layers++;
      d2d(snapshot.keys[i], past_key[i], 0, key_bytes);
      d2d(snapshot.values[i], past_value[i], 0, value_bytes);
    }
  } catch (...) {
    for (int i = 0; i < allocated_layers; i++) {
      bm_free_device(bm_handle, snapshot.keys[i]);
      bm_free_device(bm_handle, snapshot.values[i]);
    }
    throw;
  }

  const uint64_t snapshot_id = next_snapshot_id++;
  snapshot_bytes_total += snapshot.bytes;
  snapshots.emplace(snapshot_id, std::move(snapshot));
  return snapshot_id;
}

int Qwen3_5::restore_snapshot(uint64_t snapshot_id) {
  ...
  for (int i = 0; i < NUM_LAYERS; i++) {
    d2d(past_key[i], snapshot.keys[i]);
    d2d(past_value[i], snapshot.values[i]);
  }
  history_length = snapshot.history_length;
  token_length = 1;
  visited_tokens[0] = snapshot.pending_token;
  return snapshot.pending_token;
}

void Qwen3_5::release_snapshot(uint64_t snapshot_id) {
  auto it = snapshots.find(snapshot_id);
  if (it == snapshots.end()) {
    return;
  }
  for (int i = 0; i < NUM_LAYERS; i++) {
    bm_free_device(bm_handle, it->second.keys[i]);
    bm_free_device(bm_handle, it->second.values[i]);
  }
  snapshot_bytes_total -= it->second.bytes;
  snapshots.erase(it);
}

void Qwen3_5::clear_snapshots() {
  while (!snapshots.empty()) {
    release_snapshot(snapshots.begin()->first);
  }
}

uint64_t Qwen3_5::snapshot_bytes(uint64_t snapshot_id) const {
  auto it = snapshots.find(snapshot_id);
  if (it == snapshots.end()) {
    return 0;
  }
...
void Qwen3_5::init_by_names() {
  ...
  // Text-only ``without_vit`` bmodels omit the vision network.  Count the
  // fixed networks from what is actually present instead of assuming four;
  // the old assumption truncated NUM_LAYERS to 31 and then dereferenced a
  // null net_vit while loading the new 32-layer artifacts.
  const bool has_vit = is_exist("vit", net_names, num_nets);
  net_vit = has_vit ? bmrt_get_network_info(p_bmrt, "vit") : nullptr;
  auto num_blocks = num_nets - 3 - (has_vit ? 1 : 0);
...
void Qwen3_5::init(int dev_id, std::string model_path) {
...
int Qwen3_5::greedy_search(bm_device_mem_t &logits_mem) {
  std::vector<bm_tensor_t> in_tensors;
  std::vector<bm_tensor_t> out_tensors;
...
void Qwen3_5::forward_vit(ArrayFloat const &pixel_values,
                          ...
                          ArrayInt const &grid_thw, int vit_offset) {
  if (net_vit == nullptr) {
    throw std::runtime_error("This bmodel was compiled without a vision network");
  }
  ...
  bm_memcpy_s2d_partial(bm_handle, in_tensors[0].device_mem,
                        (void *)p_pixel_values.ptr, num_pixels * sizeof(float));
...
int Qwen3_5::forward_first(ArrayInt const &position_ids) {
  if (support_history) {
...
int Qwen3_5::forward_first_with_kv(ArrayInt const &position_ids) {
  ...
  // history_length includes one pending (not-yet-cached) decode token.  Drop
  // that sentinel while appending a new prefill span; otherwise every
  // successive span adds an extra phantom KV position.
  history_length = old_kvlen;
  ...
  visited_tokens[token_length] = token;
  token_length++;
  history_length = old_kvlen + 1;
  return token;
}

int Qwen3_5::select_decode_stage() {
  if (decode_stage_len.empty()) {
    return 0;
  }
  int stage_idx = 0;
...
PYBIND11_MODULE(chat, m) {
  pybind11::class_<Qwen3_5>(m, "Qwen3_5")
      .def(pybind11::init<>())
      ...
      .def("save_snapshot", &Qwen3_5::save_snapshot)
      .def("restore_snapshot", &Qwen3_5::restore_snapshot)
      .def("release_snapshot", &Qwen3_5::release_snapshot)
      .def("clear_snapshots", &Qwen3_5::clear_snapshots)
      .def("estimate_snapshot_bytes", &Qwen3_5::estimate_snapshot_bytes)
      .def("snapshot_bytes", &Qwen3_5::snapshot_bytes)
      .def("total_snapshot_bytes", &Qwen3_5::total_snapshot_bytes)
