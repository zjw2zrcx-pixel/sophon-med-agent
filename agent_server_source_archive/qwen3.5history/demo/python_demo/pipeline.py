...
class Qwen3_5():
    ...
    def run_once(self, input_str, media_path=""):
        ...
        while token not in [self.ID_IM_END] and \
                self.model.history_length < self.model.SEQLEN and \
                tok_num < self.max_new_tokens:
    ...
    def generate(self, messages, media_type="text", clear_history_flag=True):
        """Yield decoded text chunks for the persistent HTTP server."""
        ...
        inputs = self.process(messages, media_type)
        self.last_stage_ms["tokenizer"] = (time.perf_counter() - stage_started) * 1000
        token_len = inputs.input_ids.numel()
        self.last_input_token_count = token_len
        max_input_tokens = self.model.SEQLEN if self.model.support_history \
            else self.model.MAX_INPUT_LENGTH
        if token_len > max_input_tokens:
            raise RuntimeError(
                f"Input length {token_len} exceeds maximum {max_input_tokens}")
        ...
        self.last_stage_ms["prefill"] = (time.perf_counter() - stage_started) * 1000
        stage_started = time.perf_counter()
        full_word_tokens = []
        generated = 0
        while token != self.ID_IM_END and \
                self.model.history_length < self.model.SEQLEN and \
                generated < self.max_new_tokens:
            self.last_output_token_ids.append(int(token))
            full_word_tokens.append(token)
            ...
            position_ids = np.array(
                [self.max_posid, self.max_posid, self.max_posid],
                dtype=np.int32)
            token = self.model.forward_next(position_ids)
            generated += 1

        self.history_max_posid = self.max_posid + 2
        self.last_output_token_count = generated
        self.last_stage_ms["decode"] = (time.perf_counter() - stage_started) * 1000

    def prefill_token_ids(self, token_ids, start_position):
        """Append an exact text-token span to the current physical KV cache."""
        token_ids = np.asarray(token_ids, dtype=np.int32).reshape(1, -1)
        token_len = int(token_ids.size)
        if token_len <= 0:
            raise ValueError("prefill_token_ids requires at least one token")
        cached_tokens = max(0, int(self.model.history_length) - 1)
        if cached_tokens != int(start_position):
            raise RuntimeError(
                f"KV/token position mismatch: cache={cached_tokens}, "
                f"requested={start_position}")
        if cached_tokens + token_len > self.model.SEQLEN - 1:
            raise RuntimeError(
                f"Input length {cached_tokens + token_len} exceeds maximum "
                f"{self.model.SEQLEN - 1}")

        self.model.forward_embed(token_ids)
        positions = np.arange(
            start_position, start_position + token_len, dtype=np.int32)
        position_ids = np.tile(positions, 3)
        pending_token = self.model.forward_first(position_ids)
        self.last_input_token_count = cached_tokens + token_len
        self.max_posid = cached_tokens + token_len - 1
        self.history_max_posid = cached_tokens + token_len + 1
        return int(pending_token)

    def generate_from_pending(self, pending_token, prompt_token_count):
        """Decode from a restored or freshly-prefilled text-only snapshot."""
        self.last_output_token_count = 0
        self.last_output_token_ids = []
        token = int(pending_token)
        self.last_input_token_count = int(prompt_token_count)
        self.max_posid = int(prompt_token_count) - 1
        full_word_tokens = []
        generated = 0
        while token != self.ID_IM_END and \
                self.model.history_length < self.model.SEQLEN and \
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_new_tokens', type=int, default=512,
                        help='maximum number of tokens generated per turn (default: 512)')
    # yapf: enable
