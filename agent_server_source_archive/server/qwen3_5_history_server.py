...
from benchmark_api import (
    completion_limit, finish_reason as infer_finish_reason, output_token_hash,
    request_max_tokens)
from thinking_output import split_thinking_output, thinking_metadata

logger = setup_colored_logging("qwen3-5-history")
...
def make_args(cfg: dict):
    ...
    args.enable_thinking = bool(cfg.get("enable_thinking", False))
    return args


async def load_model():
    global pipeline, prefix_cache, server_state
    ...
    try:
        ...
        if not getattr(pipeline, "support_history", False):
            raise RuntimeError(
                "qwen3_5_history_server.py requires a history-capable model"
            )
        if cfg.get("disable_prefix_cache", False):
            prefix_cache = FullPrefillManager(
                pipeline, logger=logger,
                enable_thinking=cfg.get("enable_thinking", False))
            logger.info("Prefix cache disabled: every invocation uses full prefill.")
        else:
            prefix_cache = ExactPrefixCacheManager(
                pipeline,
                max_sessions=cfg.get("max_sessions", 4),
                max_snapshot_bytes=cfg.get("max_snapshot_bytes", 1024 ** 3),
                logger=logger,
                enable_thinking=cfg.get("enable_thinking", False),
            )
        server_state = "ready"
...
def _messages_from_prompt_slots(prompt_slots):
    """Convert the four-slot extension into deterministic ChatML messages.

    Slot order is a server contract and never follows JSON object iteration
    order.  History contains committed successful work; attempt contains only
    uncommitted failures for the current task.
    """
...
@app.delete("/v1/cache/sessions/{session_id}")
async def release_session_cache(session_id: str):
    """Release all TPU prefix/KV snapshots for one finished Agent session."""
    ...
    released = prefix_cache.evict(normalized, reason="conversation_ended") if prefix_cache else False
    return {"status": "released", "session_id": normalized, "prefix_cache_released": released}
...
async def chat_completions(request: Request):
    if server_state != "ready" or pipeline is None or prefix_cache is None:
        return JSONResponse(
    ...
    if prompt_slots is not None:
        if not session_id:
            return JSONResponse(
                {"error": {"message": "x-session-id is required for prompt_slots", "type": "invalid_request_error"}},
                status_code=400,
            )
        try:
            messages = _messages_from_prompt_slots(prompt_slots)
            if prompt_slots.get("image") is not None:
                raise ValueError(
                    "SUHA prefix cache is text-only; put an image description "
                    "in history or attempt"
                )
        except ValueError as exc:
            return JSONResponse(
                {"error": {"message": str(exc), "type": "invalid_request_error"}},
    ...
    if stream:
        return StreamingResponse(
            _stream_generate(
                msgs, media_type, clear_flag, chat_id, created, model_name,
                temp_image_paths, tools, prompt_slots, session_id, max_tokens),
        ...
        try:
            full_text, usage = await loop.run_in_executor(
                ...
                prompt_slots, session_id, max_tokens
            )
        except Exception as e:
        ...
        thinking, visible_text, thinking_closed = split_thinking_output(
            full_text, app.state.cfg.get("enable_thinking", False))
        # Only the post-thinking response reaches the Agent and future slots.
        clean_text, tool_calls = _parse_tool_calls_from_text(visible_text)
        ...
        if tool_calls:
            ...
            message_obj = {"role": "assistant", "content": visible_text}
            choice = {
                "index": 0,
                "message": message_obj,
        ...
        if benchmark:
            diagnostics = dict(prefix_cache.last_diagnostics)
            diagnostics.update({
                **thinking_metadata(
                    pipeline.tokenizer, thinking, thinking_closed),
...
def _request_generator(msgs, media_type, clear_flag, prompt_slots, session_id):
    if prompt_slots is not None:
        return prefix_cache.generate(
            session_id, prompt_slots, clear=clear_flag)
...
def _collect_generate(msgs, media_type, clear_flag, prompt_slots=None,
                      session_id="", max_tokens=512):
    full_text = ""
    try:
        with completion_limit(pipeline, max_tokens):
            for text_chunk in _request_generator(
                    msgs, media_type, clear_flag, prompt_slots, session_id):
...
async def _stream_generate(msgs, media_type, clear_flag, chat_id, created,
                           ...
                           prompt_slots=None, session_id="", max_tokens=512):
    queue = asyncio.Queue()
    ...
    def run_generate():
        try:
            ...
            raw_chunks = []
            with completion_limit(pipeline, max_tokens):
                gen = _request_generator(
                    msgs, media_type, clear_flag, prompt_slots, session_id)
                for text_chunk in gen:
                    if text_chunk is not None and text_chunk != "":
                        chars += len(text_chunk)
                        raw_chunks.append(text_chunk)
                        asyncio.run_coroutine_threadsafe(queue.put(("chunk", text_chunk)), loop)
            logger.info(
                f"LLM_RAW_OUTPUT ({chars} chars): [{''.join(raw_chunks)}]")
            usage = _current_usage()
            logger.info(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=(
            "../qwen3.5history/"
            "qwen3_5_4b_agent_merged_w4bf16_seq8192_bm1684x_1dev_"
            "history_dynamic_20260804_125518.bmodel"
        ),
    )
    parser.add_argument(
        "--config-path",
        type=str,
        default="../qwen3.5history/config",
    )
    parser.add_argument(
        "--module-path",
        type=str,
        default="../qwen3.5history/demo/python_demo",
    )
    ...
    parser.add_argument(
        "--max-snapshot-bytes", type=int, default=1024 ** 3)
    parser.add_argument(
        "--disable-prefix-cache", action="store_true",
        help="Disable snapshots/reuse and fully prefill every prompt_slots call")
    parser.add_argument(
        "--enable-thinking", action="store_true",
        help="Enable thinking and exclude think blocks from Agent context")
