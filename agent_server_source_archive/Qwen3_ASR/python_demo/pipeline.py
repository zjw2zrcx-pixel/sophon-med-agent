...
import qwen_asr
import librosa
from qwen_audio_utils import trim_to_whole_seconds_preserving_tail
...
class Qwen3_ASR():
    ...
    def process(self, media_path):
        ...
        # The TPU audio graph consumes fixed one-second blocks.  Do not grow
        # the block count: retain the command tail and discard only the oldest
        # fractional part of the two-second pre-roll.
        y = trim_to_whole_seconds_preserving_tail(y, self.sample_rate)
        audios = [y]
    ...
    def audio_inference(self, inputs):
        ...
        if audio_offset_list and (
            audio_offset_list[-1] + audio_tokens_per_chunk
            > self.model.MAX_INPUT_LENGTH
        ):
            raise RuntimeError(
                "Audio embedding would exceed the compiled token buffer: "
                f"end={audio_offset_list[-1] + audio_tokens_per_chunk}, "
                f"capacity={self.model.MAX_INPUT_LENGTH}"
            )
        audio_array = np.ascontiguousarray(audio_features.numpy(), dtype=np.float32)
        expected_values = n_times * 128 * self.model.AUDIO_LENGTH
        if audio_array.size != expected_values:
            raise RuntimeError(
                "Audio feature buffer size mismatch: "
                f"values={audio_array.size}, expected={expected_values}"
            )
        # do audio inference
        self.model.forward_audio(
            audio_array,
            np.asarray(audio_offset_list, dtype=np.int32),
        )
...
=================================================================""")
    ...
    def transcribe(self, context_str, audio_path, language=None, clear_history_flag=True):
        """Transcribe audio file. Yields text chunks for streaming."""
        import numpy as np, time, os
        self.input_str = context_str or ""
        ...
        if not os.path.exists(audio_path):
            ...
            return
        inputs = self.process(audio_path)
        token_len = inputs.input_ids.numel()
        if token_len > self.model.MAX_INPUT_LENGTH:
            yield "[ERROR: audio too long]"
            return
        position_ids, _ = self.get_rope_index(inputs.attention_mask)
        self.max_posid = int(position_ids.max())
        self.model.forward_embed(inputs.input_ids.squeeze(0).tolist())
        self.audio_inference(inputs)
        token = self.forward_prefill(position_ids.numpy())
        ...
        while token not in [self.ID_IM_END] and self.model.history_length < self.model.SEQLEN:
            ...
            if "\ufffd" not in word:
                ...
                if word == "<asr_text>":
                    # ``language Chinese`` before this delimiter is model
                    # protocol metadata, not spoken content.
                    text = ""
                else:
                    text += word
                full_word_tokens = []
