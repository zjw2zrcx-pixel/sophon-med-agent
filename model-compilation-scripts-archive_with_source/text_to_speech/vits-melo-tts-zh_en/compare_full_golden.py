...
decoder = sail.Engine(str(ROOT / "vits_flow_decoder_512_bm1684x_f32.bmodel"), 0, sail.IOMode.SYSIO)
...
torch.manual_seed(meta["torch_seed"])
controller = torch.jit.load(str(ROOT / "cpu_dynamic_controller.pt"), map_location="cpu").eval()
with torch.inference_mode():
    z, lengths, _ = controller(torch.from_numpy(m_p), torch.from_numpy(logs_p), torch.from_numpy(x_mask), torch.from_numpy(logw), torch.tensor([0.667]), torch.tensor([1.0]))
frames = int(lengths[0])
zpad = np.zeros((1, 192, 512), np.float32); zpad[:, :, :frames] = z.numpy()
ymask = np.zeros((1, 1, 512), np.float32); ymask[:, :, :frames] = 1
