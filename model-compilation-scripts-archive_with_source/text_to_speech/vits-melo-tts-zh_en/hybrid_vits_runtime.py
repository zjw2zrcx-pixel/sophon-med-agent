#!/usr/bin/env python3
"""Hybrid VITS inference: BM1684X encoder/decoder and CPU duration control."""
...
class HybridVitsRuntime:
    def __init__(self, model_dir: str | Path, device_id: int = 0) -> None:
        ...
        self.decoder = sail.Engine(str(root / "vits_flow_decoder_512_bm1684x_f32.bmodel"), device_id, sail.IOMode.SYSIO)
        ...
        self.controller = torch.jit.load(str(root / "cpu_dynamic_controller.pt"), map_location="cpu").eval()
    ...
    def synthesize_tokens(self, x: np.ndarray, tones: np.ndarray, sid: int = 1,
        ...
        # This export's validated audio contract fixes the latent RNG.  Without
        # it, different Torch random states can yield unintelligible samples.
        torch.manual_seed(seed)
        with torch.inference_mode():
            z, y_lengths, _ = self.controller(
                torch.from_numpy(m_p), torch.from_numpy(logs_p), torch.from_numpy(x_mask),
                torch.from_numpy(logw), torch.tensor([noise_scale]), torch.tensor([length_scale]),
            )
        frames = int(y_lengths[0])
        if frames > MAX_FRAMES:
            raise ValueError(f"predicted {frames} frames; limit is {MAX_FRAMES}; split the sentence")
        z_pad = np.zeros((1, 192, MAX_FRAMES), dtype=np.float32)
        z_pad[:, :, :frames] = z.numpy()
        y_mask = np.zeros((1, 1, MAX_FRAMES), dtype=np.float32)
        y_mask[:, :, :frames] = 1.0
