...
class LlmConverter(BaseConverter):
    ...
    def __init__(self, args, config, loader=None):
        ...
        self.speculative_mode = getattr(args, 'speculative_mode', 'none')
        self.speculative_length = getattr(args, 'speculative_length', 0)
        self.speculative_overlay = getattr(args, 'speculative_overlay', False)
        ...
        default_workers = max(cpu_count or 1, 4)
        self.max_workers = max(
            1, int(os.environ.get("LLM_COMPILE_WORKERS", str(default_workers))))
        ...
        speculative_str = (f"_spec{self.speculative_mode}{self.speculative_length}"
                           if self.speculative_mode != 'none' else "")
        if self.only_mlir:
            ...
            folder_name += speculative_str + ("_dynamic" if args.dynamic else "_static")
        else:
    ...
    def gen_all_mlir(self):
        if self.speculative_overlay:
            # Overlay builds contain only networks that are absent from an
            # already converted history-KV target.  The embedding/lm-head
            # generator also emits the selected verify head.  Strict runtime
            # deliberately reuses block_cache_* per token; embedding a fully
            # unrolled transformer layer duplicates reordered weights for
            # every row and is neither smaller nor more strictly aligned.
            ...
            if not self.only_mlir:
                ...
                if (self.llm_type in (LlmType.QWEN3_5, LlmType.QWEN3_5_MOE)
                        and os.environ.get("LLM_CACHE_VERIFY_GATE_ONLY") != "1"):
    ...
    def gen_embedding_lmhead_mlir(self):
        ...
        if self.speculative_overlay and self.speculative_mode == "strict":
            # A strict overlay carries no duplicated model weights.  Runtime
            # detects this marker and executes the existing block_cache_* and
            # lm_head networks exactly as target-only decode does.
            name = "verify_head_strict"
            marker_shape = [1, self.speculative_length]
            marker_mlir = MLIRImporter([marker_shape], [marker_shape], name,
                                       self.platform, input_types=["INT32"])
            input_op = marker_mlir.create_input_op(
                self.get_loc("max_k_marker", marker_mlir), 0)
            marker = top.ReshapeOp(
                marker_mlir.get_tensor_type(marker_shape), input_op,
                loc=self.get_loc(name + ".marker", marker_mlir),
                ip=marker_mlir.insert_point).output
            marker_mlir.create_return_op([marker])
            self.save_mlir_module(marker_mlir, name)
            return
        if isinstance(self.loader, GGUFModelHandle):
        ...
        if self.embedding_disk and not self.speculative_overlay:
            self.gen_embedding_bin(embedding_data)
        elif not self.speculative_overlay:
        ...
        def gen_verify_head_fast():
            """One dynamic GEMM and device-side top-1 for 1..max_k rows."""
            name = "verify_head_fast"
            length = self.speculative_length
            input_shape = [1, length, self.hidden_size]
            logits_shape = [1, length, self.vocab_size]
            output_shape = [1, length]
            verify_mlir = MLIRImporter([input_shape], [output_shape], name, self.platform,
                                       weight_file=f"../{lmhead_npz}")
            T = verify_mlir.get_tensor_type
            L = lambda value: self.get_loc(value, verify_mlir)
            ip = verify_mlir.insert_point
            input_op = verify_mlir.create_input_op(L("hidden_states"), 0)
            weight_op = verify_mlir.create_weight_op(
                lmhead + ".weight", [self.vocab_size, self.hidden_size])
            logits_op = top.MatMulOp(T(logits_shape),
                                     input_op,
                                     weight_op,
                                     verify_mlir.none_op,
                                     do_relu=False,
                                     ...
                                     loc=L(name + ".matmul"),
                                     ip=ip).output
            topk_op = top.TopKOp(*T([[1, length, 1], [1, length, 1]]),
                                     logits_op,
                                     axis=2,
                                     K=1,
                                     loc=L([name + ".values", name + ".indices"]),
                                     ip=ip)
            token_ids = top.ReshapeOp(T(output_shape),
                                      topk_op.indices,
                                      loc=L(name + ".reshape"),
                                      ip=ip).output
            verify_mlir.create_return_op([token_ids])
            self.save_mlir_module(verify_mlir, name)

        def gen_verify_head_strict_marker():
            """Mark a max-k model whose runtime reuses exact cache/lm_head nets."""
            name = "verify_head_strict"
            marker_shape = [1, self.speculative_length]
            verify_mlir = MLIRImporter([marker_shape], [marker_shape], name,
                                       self.platform, input_types=["INT32"])
            T = verify_mlir.get_tensor_type
            L = lambda value: self.get_loc(value, verify_mlir)
            ip = verify_mlir.insert_point
            input_op = verify_mlir.create_input_op(L("max_k_marker"), 0)
            marker = top.ReshapeOp(T(marker_shape), input_op,
                                   loc=L(name + ".marker"), ip=ip).output
            verify_mlir.create_return_op([marker])
            self.save_mlir_module(verify_mlir, name)

        def gen_spec_abi_v2_marker():
            name = "spec_abi_v2"
            marker_shape = [1, 8]
            marker_mlir = MLIRImporter([marker_shape], [marker_shape], name,
                                       self.platform, input_types=["INT32"])
            input_op = marker_mlir.create_input_op(
                self.get_loc("capability_flags", marker_mlir), 0)
            marker = top.ReshapeOp(
                marker_mlir.get_tensor_type(marker_shape), input_op,
                loc=self.get_loc(name + ".marker", marker_mlir),
                ip=marker_mlir.insert_point).output
        ...
        if not self.speculative_overlay and not self.embedding_disk:
            gen_embedding_by_length("embedding", self.max_input_length)
            gen_embedding_by_length("embedding_cache", 1)
        if not self.speculative_overlay:
            gen_lm_head()
        if self.speculative_mode in ("fast", "transactional"):
            gen_verify_head_fast()
            if self.speculative_mode == "transactional":
                gen_spec_abi_v2_marker()
        elif self.speculative_mode == "strict":
            gen_verify_head_strict_marker()
        if self.do_lora:
    ...
    def execute_tasks(self):
        ...
        self.commands.clear()
        display_args = ["--progress"] if sys.stderr.isatty() else ["--line-buffer"]
        parallel_cmd = [
            "parallel", f"-j {self.max_workers}", "--halt now,fail=1",
            *display_args,
    ...
    def compile_verify_head_fast(self):
        name = "verify_head_fast"
        ...
        self.submit_deploy_task(name, [
            f'--quantize {self.half_precision_quantize}',
            '--quant_input',
        ], dynamic=True, addr_mode='io_alone')

    def compile_verify_head_strict(self):
        name = "verify_head_strict"
        if self.register_bmodel(name, with_size=False):
            return
        self.submit_deploy_task(name, [], addr_mode='io_alone')

    def compile_block_verify_strict(self, layer_id):
        # Implemented only by converters with transactional recurrent state.
        return

    def compile_spec_abi_v2(self):
        name = "spec_abi_v2"
        if self.register_bmodel(name, with_size=False):
            return
        self.submit_deploy_task(name, [], addr_mode='io_alone')
    ...
    def combine(self):
        ...
        if total_bytes > 0 and bmodel_size > total_bytes * 1.2:
    ...
    def compile_all(self):
        if self.speculative_overlay:
            if self.speculative_mode == "fast":
                self.all_compiles.append(self.compile_verify_head_fast)
            else:
                self.all_compiles.append(self.compile_verify_head_strict)
        ...
        ## ============= main compile ================
        # --only_mlir is also the lightweight compiler-regression mode below;
        # avoid deploying multi-gigabyte tied embedding/head weights when only
        # representative block/cache/verifier networks are needed.
        if not self.only_mlir:
            if self.do_vit:
            ...
            if self.speculative_mode in ("fast", "transactional"):
                self.all_compiles.append(self.compile_verify_head_fast)
                if self.speculative_mode == "transactional":
                    self.all_compiles.append(self.compile_spec_abi_v2)
            elif self.speculative_mode == "strict":
                self.all_compiles.append(self.compile_verify_head_strict)
        ...
        if not self.only_mlir:
            for i in range(self.num_layers):
                ...
                if self.speculative_mode == "transactional":
                    self.all_compiles.append(lambda i=i: self.compile_block_verify_strict(i))
        else:
            self.all_compiles.append(lambda i=0: self.compile_block_cache(i))
            gate_only = os.environ.get("LLM_CACHE_VERIFY_GATE_ONLY") == "1"
            if not gate_only:
                self.all_compiles.insert(0, lambda i=0: self.compile_block(i))
                # The analysis build must exercise the two interfaces that make a
                # transactional target different from a normal Qwen conversion.
                # One representative linear-attention layer is sufficient to
                # validate lowering, dynamic codegen, state-bank ABI and history-KV
                # reuse without compiling all model layers.
                if self.use_history_kv:
                    self.all_compiles.append(lambda i=0: self.compile_block_kv(i))
            if self.speculative_mode == "transactional":
                self.all_compiles.append(lambda i=0: self.compile_block_verify_strict(i))
            if (not gate_only
                    and self.llm_type in (LlmType.QWEN3_5, LlmType.QWEN3_5_MOE)):
        ...
        if (len(self.decode_chunk_list) > 0
                and os.environ.get("LLM_CACHE_VERIFY_GATE_ONLY") != "1"):
            secondary_layers = (range(self.num_layers) if not self.only_mlir else [3])
            for i in secondary_layers:
                secondary_compiles.append(lambda i=i: self.compile_block_cache_chunk(i))
        for func in secondary_compiles:
            func()
        self.execute_tasks()

        # Transform/deploy has finished at this point.  The generated NPZ files
        # are no longer inputs to model_tool, and Qwen-sized conversions can
        # leave tens of gigabytes of them behind.  Remove them before combine
        # so the final bmodel does not require space for NPZ + per-network
        # bmodels + combined bmodel at the same time.
        if not self.debug:
            for dirpath, _, filenames in os.walk('.'):
